...
def main():
  ...
  addDir('[COLOR lightgreen]hplus.com.vn[/COLOR]',hplus,50,logos+'hplus.png')  
...
def h_plus():
   addDir('PHIM LẺ',hplus+'vi/categories/phim-le',51,logos+'hplus.png') 
   addDir('PHIM BỘ',hplus+'vi/categories/phim-bo',51,logos+'hplus.png') 
   addDir('TV SHOWS',hplus+'vi/categories/tv-shows',51,logos+'hplus.png') 

def hplus_dir(url):  
  content = makeRequest(url)
  match = re.compile('href="http://hplus.com.vn/vi/genre(.+?)">(.+?)<').findall(content) 
  for url, name in match:
    if 'Xem tất cả' in name: pass
    else:
      addDir(name, hplus+'vi/genre'+url,52,logos+'hplus.png')  

def hplus_movies_list(url):
  content = makeRequest(url)
  match = re.compile('href="(.+?)" style.+?url\(\'(.+?)\'\)" data-content.+?><h3 class=.+?>(.+?)</h3>').findall(content) 
  for url,thumb,name in match:
    add_Link(name,hplus+url,thumb+'?.png')
  match = re.compile('href="http://hplus.com.vn/vi/genre/index(.+?)">(\d+)<').findall(content) 
  for url,name in match:
    addDir('Trang '+name,hplus+'vi/genre/index'+url,52,logos+'hplus.png')  
...
def resolveUrl(url):
  if 'fptplay' in url:
    ...
    content=makeRequest(url)
    mediaUrl=phimmobile+re.compile('var iosUrl = "(.+?)"').findall(content)[0]		
  else:
    mediaUrl = url  
