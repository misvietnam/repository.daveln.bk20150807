...
def XMLtoM3U():
  if len(xml_file) <= 0:
    ...
    try:
      ...
      ok = xbmcgui.Dialog().ok('[COLOR orange]XML to M3U Convertor[/COLOR]', '[COLOR yellow]Congratulation![/COLOR]', '', '[COLOR lime]Done.[/COLOR]')
    except:
      pass	
...
def M3UtoXML():
  if len(m3u_file) > 0:
    try:
      ...
      ok = xbmcgui.Dialog().ok('[COLOR cyan]M3U to XML Convertor[/COLOR]', '[COLOR yellow]Congratulation![/COLOR]', '', '[COLOR lime]Done.[/COLOR]') 
    except:
      pass
  else:	
    mysettings.openSettings()  	  
